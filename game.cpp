...
Bird* Game :: createBird()
{
   ...
   // TODO: Fill this in
   Point newP;
   newP.setX(topLeft.getX());
   newP.setY(random(bottomRight.getY(),topLeft.getY()));
   int x = random(0,3);
   if(x == 0 )
     {
       newBird = new NormalBird(newP);
     }
   else if(x == 1)
     {
       newBird = new StandardBird(newP);
     }
   else
     {
       newBird = new ToughBird(newP);
     }
   return newBird;
}
